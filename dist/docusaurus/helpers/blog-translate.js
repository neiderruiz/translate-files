...
const blogTranslate = async ({
  ...
  items.forEach(async item => {
    ...
    const itemRelativePath = _path.default.relative(baseBlogDir, itemPath);
    if (_fs.default.statSync(itemPath).isDirectory()) {
      ...
      for (const locale of localeArray) {
        ...
        const routeOutputLog = _path.default.join(locale, 'docusaurus-plugin-content-blog', _path.default.dirname(itemRelativePath), item);
        if (defaultLocale === locale) {
          const baseDocsPath = _path.default.join(outputBlogDir, _path.default.dirname(itemRelativePath));
          if (!_fs.default.existsSync(baseDocsPath)) {
            _fs.default.mkdirSync(baseDocsPath, {
              recursive: true
            });
          }
          const routeFileSaveDoc = _path.default.join(outputBlogDir, _path.default.dirname(itemRelativePath), item);
...
exports.blogTranslate = blogTranslate;